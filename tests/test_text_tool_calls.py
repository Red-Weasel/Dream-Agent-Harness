...
async def test_a_call_cut_off_mid_write_says_so_instead_of_stopping_silently():
    ...
    b._client = _FakeClient([cut_round, cut_round, cut_round])   # both automatic continuations are cut off too
    ...
    assert sum("smaller pieces" in s for s in systems) == 2, "DREAM-117: two continuations in smaller pieces"
